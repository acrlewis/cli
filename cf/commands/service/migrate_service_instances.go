...
func (cmd *MigrateServiceInstances) MetaData() command_registry.CommandMetadata {
	...
	fs["f"] = &cliFlags.BoolFlag{Name: "f", Usage: i18n.T("Force migration without confirmation")}

	return command_registry.CommandMetadata{
		Name:        "migrate-service-instances",
		Description: i18n.T("Migrate service instances from one service plan to another"),
		Usage:       i18n.T("CF_NAME migrate-service-instances v1_SERVICE v1_PROVIDER v1_PLAN v2_SERVICE v2_PLAN\n\n") + migrateServiceInstanceWarning(),
		Flags:       fs,
	}
}
...
func (cmd *MigrateServiceInstances) Requirements(requirementsFactory requirements.Factory, fc flags.FlagContext) (reqs []requirements.Requirement, err error) {
	if len(fc.Args()) != 5 {
		cmd.ui.Failed(i18n.T("Incorrect Usage. Requires v1_SERVICE v1_PROVIDER v1_PLAN v2_SERVICE v2_PLAN as arguments\n\n") + command_registry.Commands.CommandUsage("migrate-service-instances"))
	}
...
func (cmd *MigrateServiceInstances) Execute(c flags.FlagContext) {
	...
	case *errors.ModelNotFoundError:
		cmd.ui.Failed(i18n.T("Plan {{.ServicePlanName}} cannot be found",
	...
	if apiErr != nil {
		...
		cmd.ui.Failed(i18n.T("Plan {{.ServicePlanName}} has no service instances to migrate", map[string]interface{}{"ServicePlanName": terminal.EntityNameColor(v1.String())}))
		return
	}
	...
	if !force {
		response := cmd.ui.Confirm(
			i18n.T("Really migrate {{.ServiceInstanceDescription}} from plan {{.OldServicePlanName}} to {{.NewServicePlanName}}?>",
				map[string]interface{}{
	...
	cmd.ui.Say(i18n.T("Attempting to migrate {{.ServiceInstanceDescription}}...", map[string]interface{}{"ServiceInstanceDescription": serviceInstancesPhrase}))
	...
	cmd.ui.Say(i18n.T("{{.CountOfServices}} migrated.", map[string]interface{}{"CountOfServices": pluralizeServiceInstances(changedCount)}))
	cmd.ui.Ok()
...
func pluralizeServiceInstances(count int) string {
	var phrase string
	if count == 1 {
		phrase = i18n.T("service instance")
	} else {
		phrase = i18n.T("service instances")
	}

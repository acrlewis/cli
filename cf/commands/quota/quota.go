...
func (cmd *showQuota) MetaData() command_registry.CommandMetadata {
	return command_registry.CommandMetadata{
		Name:        "quota",
		Usage:       i18n.T("CF_NAME quota QUOTA"),
		Description: i18n.T("Show quota info"),
	}
}
...
func (cmd *showQuota) Execute(c flags.FlagContext) {
	quotaName := c.Args()[0]
	cmd.ui.Say(i18n.T("Getting quota {{.QuotaName}} info as {{.Username}}...", map[string]interface{}{"QuotaName": quotaName, "Username": cmd.config.Username()}))
	...
	table.Add(i18n.T("Total Memory"), formatters.ByteSize(quota.MemoryLimit*formatters.MEGABYTE))
	table.Add(i18n.T("Instance Memory"), megabytes)
	table.Add(i18n.T("Routes"), fmt.Sprintf("%d", quota.RoutesLimit))
	table.Add(i18n.T("Services"), servicesLimit)
	table.Add(i18n.T("Paid service plans"), formatters.Allowed(quota.NonBasicServicesAllowed))
	table.Print()
}

...
func (cmd *UpdateSecurityGroup) MetaData() command_registry.CommandMetadata {
	primaryUsage := i18n.T("CF_NAME update-security-group SECURITY_GROUP PATH_TO_JSON_RULES_FILE")
	secondaryUsage := i18n.T("   The provided path can be an absolute or relative path to a file.\n   It should have a single array with JSON objects inside describing the rules.")
	tipUsage := i18n.T("TIP: Changes will not apply to existing running applications until they are restarted.")
	return command_registry.CommandMetadata{
		Name:        "update-security-group",
		Description: i18n.T("Update a security group"),
...
func (cmd *UpdateSecurityGroup) Execute(context flags.FlagContext) {
	...
	cmd.ui.Say(i18n.T("Updating security group {{.security_group}} as {{.username}}",

...
func (cmd *EnableSSH) MetaData() command_registry.CommandMetadata {
	return command_registry.CommandMetadata{
		Name:        "enable-ssh",
		Description: i18n.T("enable ssh for the application"),
		Usage:       i18n.T("CF_NAME enable-ssh APP_NAME"),
	}
}
...
func (cmd *EnableSSH) Execute(fc flags.FlagContext) {
	...
	if app.EnableSsh {
		cmd.ui.Say(fmt.Sprintf(i18n.T("ssh support is already enabled")+" for '%s'", app.Name))
		return
	}

	cmd.ui.Say(fmt.Sprintf(i18n.T("Enabling ssh support for '%s'..."), app.Name))
	cmd.ui.Say("")

	enable := true
	updatedApp, err := cmd.appRepo.Update(app.Guid, models.AppParams{EnableSsh: &enable})
	if err != nil {
		cmd.ui.Failed(i18n.T("Error enabling ssh support for ") + app.Name + ": " + err.Error())
	}

	if updatedApp.EnableSsh {
		cmd.ui.Ok()
	} else {
		cmd.ui.Failed(i18n.T("ssh support is not enabled for ") + app.Name)
	}
}

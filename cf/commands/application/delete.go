...
func (cmd *DeleteApp) MetaData() command_registry.CommandMetadata {
	...
	fs["r"] = &cliFlags.BoolFlag{Name: "r", Usage: i18n.T("Also delete any mapped routes")}

	return command_registry.CommandMetadata{
		Name:        "delete",
		ShortName:   "d",
		Description: i18n.T("Delete an app"),
		Usage:       i18n.T("CF_NAME delete APP_NAME [-f -r]"),
		Flags:       fs,
	}
}
...
func (cmd *DeleteApp) Execute(c flags.FlagContext) {
	...
	if !c.Bool("f") {
		response := cmd.ui.ConfirmDelete(i18n.T("app"), appName)
		if !response {
			return
		}
	}

	cmd.ui.Say(i18n.T("Deleting app {{.AppName}} in org {{.OrgName}} / space {{.SpaceName}} as {{.Username}}...",
	...
	case *errors.ModelNotFoundError:
		cmd.ui.Ok()
		cmd.ui.Warn(i18n.T("App {{.AppName}} does not exist.", map[string]interface{}{"AppName": appName}))
		return
	default:

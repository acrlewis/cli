...
func (cmd *CopySource) MetaData() command_registry.CommandMetadata {
	...
	fs["no-restart"] = &cliFlags.BoolFlag{Name: "no-restart", Usage: i18n.T("Override restart of the application in target environment after copy-source completes")}
	fs["o"] = &cliFlags.StringFlag{Name: "o", Usage: i18n.T("Org that contains the target application")}
	fs["s"] = &cliFlags.StringFlag{Name: "s", Usage: i18n.T("Space that contains the target application")}

	return command_registry.CommandMetadata{
		Name:        "copy-source",
		Description: i18n.T("Make a copy of app source code from one application to another.  Unless overridden, the copy-source command will restart the application."),
		Usage:       i18n.T("   CF_NAME copy-source SOURCE-APP TARGET-APP [-o TARGET-ORG] [-s TARGET-SPACE] [--no-restart]\n"),
		Flags:       fs,
	}
}
...
func (cmd *CopySource) Requirements(requirementsFactory requirements.Factory, fc flags.FlagContext) (reqs []requirements.Requirement, err error) {
	if len(fc.Args()) != 2 {
		cmd.ui.Failed(i18n.T("Incorrect Usage. Requires SOURCE-APP TARGET-APP as arguments\n\n") + command_registry.Commands.CommandUsage("copy-source"))
	}
...
func (cmd *CopySource) Execute(c flags.FlagContext) {
	...
	if targetOrg != "" && targetSpace == "" {
		cmd.ui.Failed(i18n.T("Please provide the space within the organization containing the target application"))
	}
	...
	cmd.ui.Say(copyStr)
	cmd.ui.Say(i18n.T("Note: this may take some time"))
	cmd.ui.Say("")
...
func (cmd *CopySource) findSpaceGuid(targetOrg, targetSpace string) string {
	...
	if !foundSpace {
		cmd.ui.Failed(fmt.Sprintf(i18n.T("Could not find space {{.Space}} in organization {{.Org}}",
...
func buildCopyString(sourceAppName, targetAppName, targetOrgName, targetSpaceName, username string) string {
	return fmt.Sprintf(i18n.T("Copying source from app {{.SourceApp}} to target app {{.TargetApp}} in org {{.OrgName}} / space {{.SpaceName}} as {{.Username}}...",
		map[string]interface{}{

...
func (cmd *DeleteSpace) MetaData() command_registry.CommandMetadata {
	...
	return command_registry.CommandMetadata{
		Name:        "delete-space",
		Description: i18n.T("Delete a space"),
		Usage:       i18n.T("CF_NAME delete-space SPACE [-f]"),
		Flags:       fs,
	}
}
...
func (cmd *DeleteSpace) Execute(c flags.FlagContext) {
	...
	if !c.Bool("f") {
		if !cmd.ui.ConfirmDelete(i18n.T("space"), spaceName) {
			return
		}
	}

	cmd.ui.Say(i18n.T("Deleting space {{.TargetSpace}} in org {{.TargetOrg}} as {{.CurrentUser}}...",
	...
	if cmd.config.SpaceFields().Guid == space.Guid {
		...
		cmd.ui.Say(i18n.T("TIP: No space targeted, use '{{.CfTargetCommand}}' to target a space",

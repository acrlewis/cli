...
func (cmd *AllowSpaceSSH) MetaData() command_registry.CommandMetadata {
	return command_registry.CommandMetadata{
		...
		Description: i18n.T("allow SSH access for the space"),
		Usage:       i18n.T("CF_NAME allow-space-ssh SPACE_NAME"),
	}
}
...
func (cmd *AllowSpaceSSH) Execute(fc flags.FlagContext) {
	...
	if space.AllowSSH {
		cmd.ui.Say(fmt.Sprintf(i18n.T("ssh support is already enabled in space ")+"'%s'", space.Name))
		return
	}

	cmd.ui.Say(fmt.Sprintf(i18n.T("Enabling ssh support for space '%s'..."), space.Name))
	cmd.ui.Say("")

	err := cmd.spaceRepo.SetAllowSSH(space.Guid, true)
	if err != nil {
		cmd.ui.Failed(i18n.T("Error enabling ssh support for space ") + space.Name + ": " + err.Error())
	}

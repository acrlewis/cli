...
func (cmd *RenameSpace) MetaData() command_registry.CommandMetadata {
	return command_registry.CommandMetadata{
		Name:        "rename-space",
		Description: i18n.T("Rename a space"),
		Usage:       i18n.T("CF_NAME rename-space SPACE NEW_SPACE"),
	}
}
...
func (cmd *RenameSpace) Requirements(requirementsFactory requirements.Factory, fc flags.FlagContext) (reqs []requirements.Requirement, err error) {
	if len(fc.Args()) != 2 {
		cmd.ui.Failed(i18n.T("Incorrect Usage. Requires SPACE_NAME NEW_SPACE_NAME as arguments\n\n") + command_registry.Commands.CommandUsage("rename-space"))
	}
...
func (cmd *RenameSpace) Execute(c flags.FlagContext) {
	...
	newName := c.Args()[1]
	cmd.ui.Say(i18n.T("Renaming space {{.OldSpaceName}} to {{.NewSpaceName}} in org {{.OrgName}} as {{.CurrentUser}}...",

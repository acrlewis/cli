...
func (cmd *Authenticate) MetaData() command_registry.CommandMetadata {
	return command_registry.CommandMetadata{
		Name:        "auth",
		Description: i18n.T("Authenticate user non-interactively"),
		Usage: i18n.T("CF_NAME auth USERNAME PASSWORD\n\n") +
			terminal.WarningColor(i18n.T("WARNING:\n   Providing your password as a command line option is highly discouraged\n   Your password may be visible to others and may be recorded in your shell history\n\n")) + i18n.T("EXAMPLE:\n") + i18n.T("   CF_NAME auth name@example.com \"my password\" (use quotes for passwords with a space)\n") + i18n.T("   CF_NAME auth name@example.com \"\\\"password\\\"\" (escape quotes if used in password)"),
	}
}
...
func (cmd *Authenticate) Requirements(requirementsFactory requirements.Factory, fc flags.FlagContext) (reqs []requirements.Requirement, err error) {
	if len(fc.Args()) != 2 {
		cmd.ui.Failed(i18n.T("Incorrect Usage. Requires 'username password' as arguments\n\n") + command_registry.Commands.CommandUsage("auth"))
	}
...
func (cmd *Authenticate) Execute(c flags.FlagContext) {
	...
	cmd.ui.Say(i18n.T("API endpoint: {{.ApiEndpoint}}",
	...
	cmd.ui.Ok()
	cmd.ui.Say(i18n.T("Use '{{.Name}}' to view or set your target org and space",

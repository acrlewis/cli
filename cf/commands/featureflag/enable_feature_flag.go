...
func (cmd *EnableFeatureFlag) MetaData() command_registry.CommandMetadata {
	return command_registry.CommandMetadata{
		...
		Description: i18n.T("Enable the use of a feature so that users have access to and can use the feature."),
		Usage:       i18n.T("CF_NAME enable-feature-flag FEATURE_NAME"),
	}
}
...
func (cmd *EnableFeatureFlag) Execute(c flags.FlagContext) {
	...
	cmd.ui.Ok()
	cmd.ui.Say("")
	cmd.ui.Say(i18n.T("Feature {{.FeatureFlag}} Enabled.", map[string]interface{}{
		"FeatureFlag": terminal.EntityNameColor(flag)}))
	return
}

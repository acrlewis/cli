...
func (cmd *SetSpaceRole) MetaData() command_registry.CommandMetadata {
	return command_registry.CommandMetadata{
		Name:        "set-space-role",
		Description: i18n.T("Assign a space role to a user"),
		Usage: i18n.T("CF_NAME set-space-role USERNAME ORG SPACE ROLE\n\n") +
			i18n.T("ROLES:\n") +
...
func (cmd *SetSpaceRole) SetSpaceRole(space models.Space, role, userGuid, userName string) (err error) {
	cmd.ui.Say(i18n.T("Assigning role {{.Role}} to user {{.TargetUser}} in org {{.TargetOrg}} / space {{.TargetSpace}} as {{.CurrentUser}}...",
